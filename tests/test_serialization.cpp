...
TEST_CASE("serialize complex struct") {
  ...
  SECTION("Person struct") {
    Person p{.name = "Alice", .age = 30, .hobbies = {"reading", "coding", "gaming"}};
...
TEST_CASE("serialize nested struct") {
  ...
  SECTION("Company struct") {
    Company company{.name = "ACME Corp",
                    ...
                    .employees = {{"Alice", 30, {"coding"}}, {"Bob", 25, {"design"}}}};
...
TEST_CASE("serialize optional") {
  ...
  SECTION("Task with optional") {
    Task task1{.title = "Task 1", .priority = Priority::High, .assignee = "Alice"};

    Task task2{.title = "Task 2", .priority = Priority::Low, .assignee = std::nullopt};
...
TEST_CASE("file I/O") {
  ...
  SECTION("save and load json") {
    Person person{.name = "Test Person", .age = 25, .hobbies = {"test1", "test2"}};

...
void test_expected_and_then() {
  auto result = expected<int, std::string>::ok(10);
  auto chained = result.and_then([](int x) { return expected<int, std::string>::ok(x * 2); });
...
void test_try_catch() {
  auto result = try_catch([]() { return 42; });
  assert(result.is_ok());
  assert(result.value() == 42);

  auto err_result = try_catch([]() -> int { throw std::runtime_error("test error"); });

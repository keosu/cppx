...
int main() {
  ...
  logger->info("1. Thread pool with tasks:");
  {
    ...
    for (size_t i = 0; i < futures.size(); ++i) {
      std::cout << "   fib(" << (20 + i) << ") = " << futures[i].get() << "\n";
    }
  }
  logger->info("\n");
  ...
  parallel_for(0, 100, [&data](int i) { data[i] = i * i; });
  ...
  logger->info("4. Blocking queue:");
  {
    ...
    std::thread producer([&queue, logger]() {
      std::vector<std::string> items = {"task1", "task2", "task3", "task4", "task5"};
  ...
  parallel_invoke([logger]() { logger->info("   Task 1 running"); },

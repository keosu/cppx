...
class expected {
 public:
  ...
  static expected ok(const T& value) { return expected(ok_tag{}, value); }

  static expected ok(T&& value) { return expected(ok_tag{}, std::move(value)); }

  // Create an error result
  static expected err(const E& error) { return expected(err_tag{}, error); }

  static expected err(E&& error) { return expected(err_tag{}, std::move(error)); }
  ...
  [[nodiscard]] bool is_err() const noexcept { return std::holds_alternative<E>(storage_); }
...
class expected<void, E> {
 public:
  ...
  static expected ok() { return expected(ok_tag{}); }
...
export class error_info {
 public:
  ...
  explicit error_info(std::string message) : message_(std::move(message)) {}
...
export inline auto ok() { return expected<void, error_info>::ok(); }
...
export class exception_wrapper : public std::exception {
 public:
  explicit exception_wrapper(error_info info) : info_(std::move(info)) {}

  [[nodiscard]] const char* what() const noexcept override { return info_.message().c_str(); }
...
auto try_catch(F&& f) -> expected<std::invoke_result_t<F>, error_info> {
  try {
    ...
    return expected<std::invoke_result_t<F>, error_info>::err(error_info(e.what()));
  } catch (...) {
    return expected<std::invoke_result_t<F>, error_info>::err(error_info("Unknown exception"));
  }
}

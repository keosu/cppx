...
public:
    explicit logger(std::string name)
        : name_(std::move(name)), level_(log_level::info)
    {
    ...
    void debug(std::string_view message) {
        log(log_level::debug, std::string(message));
    }
    
    void info(std::string_view message) {
        log(log_level::info, std::string(message));
    }
    
    void warn(std::string_view message) {
        log(log_level::warn, std::string(message));
    }
    
    void error(std::string_view message) {
        log(log_level::error, std::string(message));
    }
        ...
        if (console_sink_) {
            if (console_sink_->use_color()) {
                formatted = std::format(
                    ...
                    log_level_color(level), level_str, ansi::reset,
                    ansi::cyan, name_, ansi::reset,
                    message
                );
            } else {
                formatted = std::format(
                    "[{}] [{}] [{}] {}",
                    timestamp, level_str, name_, message
                );
            }
...
public:
    log_scope(std::shared_ptr<logger> logger, std::string scope_name)
        : logger_(std::move(logger)), scope_name_(std::move(scope_name))

...
// Write text to file
export expected<void, error_info> write_file(
    const fs::path& path,
...
export expected<void, error_info> write_file_binary(
    const fs::path& path,
...
// Write lines to file
export expected<void, error_info> write_lines(
    const fs::path& path,
    ...
    try {
        if (!fs::exists(path)) {
            return expected<size_t, error_info>::err(
                error_info("File does not exist: " + path.string())
            );
        }
...
// Copy file
export expected<void, error_info> copy_file(
...
// Move/rename file
export expected<void, error_info> move_file(
    const fs::path& source,
    const fs::path& dest
) {
    try {
...
export expected<void, error_info> remove_file(const fs::path& path) {
    try {
        if (!fs::remove(path)) {
            return expected<void, error_info>::err(
                error_info("Failed to remove file: " + path.string())
            );
        }
        return expected<void, error_info>::ok();
    } catch (const std::exception& e) {

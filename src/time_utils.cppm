...
export std::string now_iso8601() {
    return format_iso8601(system_clock::now());
}

export std::string now_rfc3339() {
    return format_rfc3339(system_clock::now());
}
...
export expected<system_clock::time_point, error_info> parse_iso8601(std::string_view str) {
    ...
    if (time == -1) {
        return expected<system_clock::time_point, error_info>::err(
            error_info("Invalid time value")
        );
    }
    
    return expected<system_clock::time_point, error_info>::ok(
        system_clock::from_time_t(time)
    );
}

// Parse custom time format
export expected<system_clock::time_point, error_info> parse_time(
    std::string_view str,
    std::string_view format = "%Y-%m-%d %H:%M:%S"
) {
    std::tm tm = {};
...
export auto time_scope(std::string name, auto callback) {
    return scope_timer(std::move(name), 
        [callback = std::move(callback)](milliseconds duration) {
            callback(duration);
        }
    );
}

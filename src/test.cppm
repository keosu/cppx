...
public:
    ...
    void add_assertion(assertion_result result) {
        assertions_.push_back(std::move(result));
    }
    
    const std::vector<assertion_result>& get_assertions() const {
        return assertions_;
    }
...
public:
    ...
    void register_test(test_case_info info) {
        test_cases_.push_back(std::move(info));
    }
    
    const std::vector<test_case_info>& get_tests() const {
        return test_cases_;
    }
...
public:
    test_registrar(const std::string& name, const std::string& tags,
                   std::function<void()> body, const char* file, int line) {
...
private:
    ...
    void print_summary() {
        ...
        logger_->info("Passed: {} ({}%)", passed_count_, 
                     total > 0 ? (passed_count_ * 100 / total) : 0);
...
// Assertion helpers
export inline void record_assertion(bool condition, const char* expr, 
...
export inline void record_check(bool condition, const char* expr,
                                const char* file, int line,
                                const std::string& message = "") {
    assertion_result result;
    result.passed = condition;
...
inline void register_test_case(const char* name, const char* tags, Func&& func, 
                               const char* file, int line) {
    test_registry::instance().register_test({
        name, tags, std::forward<Func>(func), file, line
    });
}
